...
#==========
setup_controller_primary_node() {
	echo "start setup DCE controller"
	bash -c "$(docker run --rm -i daocloud.io/daocloud/dce install -q)"
}
setup_controller_secondary_node() {
	bash -c "$(docker run --rm -i daocloud.io/daocloud/dce install -q)"
}
setup_compute_node() {
	controller_ip=$1
	echo "start setup compute node " $controller_ip
	bash -c "$(docker run --rm -i daocloud.io/daocloud/dce join $controller_ip -q)"
}
setup_dce() {
	setup_controller_primary_node
}
	...
	case "$lsb_dist" in
		...
		ubuntu|debian)
			...
			cat >&1 <<-'EOF'
																...
																##########################
																Start set up DCE on ubuntu
																##########################

					http://docs.daocloud.io/daocloud-enterprise/install-for-production
